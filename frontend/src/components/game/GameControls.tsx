    ...
    <div className={cn("grid md:grid-cols-4 gap-4 p-4 glass-card rounded-xl items-center", className)}>
      ...
      <div className="flex flex-col md:flex-row gap-4 md:col-span-2 lg:col-span-1">
        <div className="flex-1 flex flex-col items-center justify-center p-3 rounded-lg bg-black/20 border border-white/5">
          <p className="text-[10px] text-muted-foreground uppercase tracking-widest mb-1">{t('game.score')}</p>
          <p className="text-2xl font-bold text-primary font-mono">{score}</p>
        </div>
        <div className="flex-1 flex flex-col items-center justify-center p-3 rounded-lg bg-black/20 border border-white/5">
          <p className="text-[10px] text-muted-foreground uppercase tracking-widest mb-1">{t('game.highScore')}</p>
      ...
      <div className="space-y-1 md:col-span-4 lg:col-span-2">
        <p className="text-[10px] text-muted-foreground uppercase tracking-widest pl-1 mb-1">{t('game.difficulty')}</p>
          ...
          {(Object.keys(DIFFICULTY_CONFIG) as GameDifficulty[]).map((diff) => {
            ...
            return (
              <Button
                ...
                disabled={status !== 'idle'}
                title={`${config.label} (×${config.scoreMultiplier} ${t('game.scoreMultiplier')})`}
              >
      ...
      <div className="hidden lg:flex md:col-span-4 justify-between text-[10px] text-muted-foreground px-2 border-t border-white/5 pt-2 mt-1">
        <span>{t('game.instructions.move', { arrows: <kbd className="bg-black/30 px-1 rounded text-foreground font-mono">Arrows</kbd>, wasd: <kbd className="bg-black/30 px-1 rounded text-foreground font-mono">WASD</kbd> })}</span>
        <span>{t('game.instructions.pause', { space: <kbd className="bg-black/30 px-1 rounded text-foreground font-mono">Space</kbd> })}</span>
      </div>
    </div>
  );
}
